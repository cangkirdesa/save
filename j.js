...
var ConverterSelector = (function() {
	...
	ConverterSelector.getConvMeta = function(dom, context, defConvMeta) {
		var _this = this;
		return $.Deferred(function(def) {
			context.getFormatsPro().then(function(srcFormats) {
				if(defConvMeta && Utils.arrContains(Utils.getSrcFormats(defConvMeta), srcFormats)) {
					def.resolve(defConvMeta).promise();
				} else {
					Utils.getMetas(dom.bodyDom.getConversionInfoPath(), srcFormats).then(function(metas) {
						if(metas.length > 1) metas = metas.filter(function(m) {
							return m.id !== "index";
						});
						if(metas.length === 0) {
							def.reject(srcFormats[0]).promise();
						} else if(metas.length === 1) {
							window.history.pushState(metas[0].name, metas[0].name, dom.bodyDom.getLocalizationPrefix() + "/" + metas[0].translatedId);
							def.resolve(metas[0]).promise();
						} else {
							_this.show(dom, context, metas).then(function(meta) {
								return def.resolve(meta);
							});
						}
					});
				}
	...
	ConverterSelector.show = function(dom, context, metas) {
		return $.Deferred(function(def) {
			def.always(function() {
				return dom.hide();
			});
			dom.bodyDom.trigger("fileInputSetEnabled", false);
			context.getTfles().forEach(function(tFle) {
				tFle.getFlesPro().then(function(fles) {
					return fles.forEach(function(fle, i) {
						fle.getNamePro().then(function(fileName) {
							return dom.addFile(i.toString(), fileName);
						});
						fle.getThumbnailUrlPro().then(function(url) {
							return Utils.preloadImage(url);
						}).then(function(url) {
							return dom.setFileThumbnail(i.toString(), url);
						}, function() {
							return dom.toggleFileSpinner(i.toString(), false);
						});
					});
				});
			});
			var sortedMetas = metas.sort(function(a, b) {
				var result = a.groupSortIndex - b.groupSortIndex;
				if(result === 0) result = a.sortIndex - b.sortIndex;
				return result;
			});
			sortedMetas.forEach(function(m) {
				return dom.addFormat(m.id, m.name, m.cssClass);
			});
			dom.show();
			dom.bind("formatSelected", function(e, id) {
				var meta = metas.filter(function(m) {
					return m.id === id;
				})[0];
				window.history.pushState(meta.name, meta.name, dom.bodyDom.getLocalizationPrefix() + "/" + meta.translatedId);
				def.resolve(meta);
			});
		}).promise();
	};
...
var ManagerDom = (function(_super) {
	__extends(ManagerDom, _super);

	function ManagerDom(bodyDom) {
		var _this_1 = _super.call(this) || this;
		_this_1.sortableEnabled = false;
		_this_1.bodyDom = bodyDom;
		_this_1.elManagerContainer = _this_1.bodyDom.getBody().find(".manager-container");
		_this_1.elFiles = _this_1.elManagerContainer.find(".files");
		_this_1.elActionsContainer = _this_1.elManagerContainer.find(".actions-container");
		_this_1.elSidePanel = _this_1.bodyDom.getBody().find('#actions-container');
		_this_1.elBackToManager = _this_1.bodyDom.getBody().find('.js-back-to-manager');
		_this_1.elProFeaturesSidePanel = _this_1.bodyDom.getBody().find('#pro-feature');
		_this_1.bodyDom.getBody().on("keydown", function(e) {
			return e.ctrlKey && e.keyCode === 65 ? (_this_1.trigger("fileSelectAll"), false) : true;
		});
		_this_1.elFiles.on("click", ".file-item-wrapper", function(e) {
			return(_this_1.trigger("fileClick", $(e.currentTarget).parent().attr("id"), e.shiftKey), false);
		});
		_this_1.elFiles.on("click", ".rotate", function(e) {
			return(_this_1.trigger("rotateFileClick", $(e.currentTarget).closest(".js-file-item").attr("id")), false);
		});
		_this_1.elFiles.on("click", ".remove", function(e) {
			return(_this_1.trigger("removeFileClick", $(e.currentTarget).closest(".js-file-item").attr("id")), false);
		});
		_this_1.elFiles.on("click", ".split", function(e) {
			return(_this_1.trigger("splitFileClick", $(e.currentTarget).closest(".js-file-item").attr("id")), false);
		});
		$(window).add(_this_1.bodyDom.elWorkArea).on("scroll resize fileCntChange", function() {
			return _this_1.getFirstVisibleFileIds().forEach(function(id) {
				return _this_1.trigger("newVisibleFile", id);
			});
		});
		var _this = _this_1;
		_this_1.elBackToManager.on('click', function() {
			_this.bodyDom.showSidePanel(_this.elSidePanel);
		});
		_this_1.bind("newVisibleFile", function() {
			return _this_1.elFiles.find('.controls .move').length > 0 && !_this_1.sortableEnabled ? _this_1.enableSortable() : null;
		});
		_this_1.bodyDom.elWorkArea.on('click', '.js-range-select-all', function(e) {
			return _this_1.trigger('fileSelectAll');
		});
		_this_1.elActionsContainer.on('click', '.js-toggle-manager-options', function(e) {
			return _this_1.toggleCustomOptionsMobile(e);
		});
		_this_1.resizeSidePanel();
		window.addEventListener('resize', function() {
			return _this_1.resizeSidePanel();
		});
		return _this_1;
	}
	ManagerDom.prototype.toggleCustomOptionsMobile = function(e) {
		$(e.currentTarget).parent().toggleClass('open').closest('.side-panel').toggleClass('mobile-full-screen');
		if(window.innerWidth <= 930) window.dispatchEvent(new Event('resize'));
	};
	ManagerDom.prototype.resizeSidePanel = function() {
		setTimeout(function() {
			var actionsContainer = jQuery('#actions-container')[0];
			if($(actionsContainer).hasClass('open') && actionsContainer.scrollHeight > actionsContainer.clientHeight) $(actionsContainer).find('.proceed-btn-wrapper').addClass('sticky').parent().find('#advanced-options').toggleClass('extra-padding', true);
	...
	ManagerDom.prototype.getParamVals = function() {
		...
		elParams.each(function(idx, el) {
			return $(el).attr("name").split(" ").forEach(function(name) {
				return result[name] = $(el).val();
			});
		});
		return result;
	};
	ManagerDom.prototype.show = function(srcFormats, paramHtml) {
		this.elManagerContainer.removeClass("hidden");
		this.elFiles.removeClass("hidden");
		this.setActions(paramHtml);
	};
	ManagerDom.prototype.isVisible = function() {
		return !this.elManagerContainer.hasClass("hidden");
	};
	ManagerDom.prototype.hide = function() {
		var _this_1 = this;
		this.bodyDom.closeSidePanel(this.elSidePanel);
		this.elFiles.addClass("hidden");
		setTimeout(function() {
			_this_1.elManagerContainer.addClass("hidden");
			_this_1.elActionsContainer.addClass("hidden");
		}, 400);
	};
	...
	ManagerDom.prototype.showFileControls = function(tFleId, c) {
		...
		this.bodyDom.showFileActions(c.getConvMeta());
	};
	ManagerDom.prototype.setFileThumbnail = function(tFleId, url) {
		this.hideSpinnerPlaceholder(tFleId);
		this.getFileThumbnailEl(tFleId).attr("src", url).removeClass("hidden");
		this.getFilePlaceholderEl(tFleId).addClass("hidden");
	};
	ManagerDom.prototype.setSpinner = function(tFleId) {
		this.findById(tFleId).find(".file-item-main").children().addClass("hidden");
		this.findById(tFleId).find(".spinner").removeClass("hidden");
	};
	ManagerDom.prototype.setFileThumbnailPlaceholder = function(tFleId, format) {
		this.hideSpinnerPlaceholder(tFleId);
		this.getFilePlaceholderEl(tFleId).removeClass("hidden").addClass("ready img-file-" + format);
	};
	ManagerDom.prototype.findById = function(tFleId) {
		return this.elFiles.children("#" + tFleId);
	};
	ManagerDom.prototype.getFileThumbnailEl = function(tFleId) {
		return this.findById(tFleId).find(".thumbnail");
	};
	ManagerDom.prototype.getFilePlaceholderEl = function(tFleId) {
		return this.findById(tFleId).find(".placeholder");
	};
	ManagerDom.prototype.getFileControlsEl = function(tFleId) {
		return this.findById(tFleId).find(".controls");
	};
	ManagerDom.prototype.rotateFile = function(tFleId, angle) {
		var thumb = this.findById(tFleId).find(".rotatable");
		thumb.parent().parent().parent().toggleClass('rotated');
		thumb.css("transform", "rotate(" + angle + "deg)");
	};
	ManagerDom.prototype.removeFile = function(tFleId) {
		var elFle = this.findById(tFleId).css({
	...
	ManagerDom.prototype.addSpinner = function(id) {
		var elSpinner = $(this.elFiles.data("template-spinner").replace("{{id}}", id));
		this.elFiles.append(elSpinner);
	};
	ManagerDom.prototype.replaceFileWithSpinner = function(id) {
		var elSpinner = $(this.elFiles.data("template-spinner").replace("{{id}}", id));
		this.findById(id).replaceWith(elSpinner);
	};
	ManagerDom.prototype.removeSpinner = function(id) {
		$("#" + id).remove();
	};
	ManagerDom.prototype.hideSpinnerPlaceholder = function(tFleId) {
		$("#" + tFleId + " .spinner").addClass('hidden');
	};
	ManagerDom.prototype.setButtonLable = function(lable) {
		var newLable = this.elConvertBtn.text().replace(/{{lable}}/g, lable);
		this.elConvertBtn.text(newLable);
	};
	ManagerDom.prototype.enableFileActions = function(enable) {
		var _this_1 = this;
		if(enable === void 0) {
			enable = true;
		}
		if(enable) this.elActionsContainer.removeClass("hidden");
		else setTimeout(function() {
			return _this_1.elActionsContainer.addClass("hidden");
		}, 500);
		this.bodyDom.showSidePanel(this.elSidePanel);
		this.elActionsContainer.toggleClass("disabled", !enable);
	};
	ManagerDom.prototype.getFirstVisibleFileIds = function() {
		return this.elFiles.find(".js-file-item").filter(function(i, e) {
			return !$(e).prop("thumbnailGenerated") && Utils.elIsInViewport(e);
		}).map(function(i, e) {
			$(e).prop("thumbnailGenerated", true);
			return $(e).attr("id");
	...
	ManagerDom.prototype.setFileSelected = function(tFleId, selected) {
		this.findById(tFleId).toggleClass("selected", selected);
	};
	ManagerDom.prototype.hideFileUpload = function() {
		jQuery('.file-upload-shrink').addClass('fade-out');
	};
	ManagerDom.prototype.setSplitGroups = function(context) {
		var _this_1 = this;
		if(jQuery('.l-pdf-split').length) {
			var files_1 = context.getTfles();
			jQuery('.group-inner, .group-outer-left, .group-outer-right').removeClass('group-inner').removeClass('group-outer-left').removeClass('group-outer-right');
			files_1.forEach(function(tfle, i) {
				var elFile = _this_1.findById(tfle.getId());
				if(i == 0 && files_1.length > 0) {
					if(files_1[0].selected) {
						elFile.addClass("group-single");
					} else {
						elFile.addClass("group-outer-left");
					}
				}
				if(i > 0 && i < (files_1.length - 1)) {
					if(!tfle.selected && !files_1[i - 1].selected) {
						elFile.addClass("group-inner");
					} else if(tfle.selected && !files_1[i - 1].selected) {
						elFile.addClass("group-outer-right");
					} else if(!tfle.selected && files_1[i - 1].selected) {
						elFile.addClass("group-outer-left");
					} else if(tfle.selected && files_1[i - 1].selected) {
						elFile.addClass("group-single");
